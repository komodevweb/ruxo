import React from "react";

interface Feature {
     text: string;
     subtext?: string;
}

interface PricingCardProps {
     plan: string;
     price: string;
     originalPrice?: string | null;
     duration: string;
     billText?: string;
     planInlude?: string;
     description: string;
     features: Feature[];
     buttonText: string;
     buttonDisabled?: boolean;
     highlight?: boolean;
     onClick?: (e?: React.MouseEvent) => void;
     isDimmed?: boolean;
}

const CheckIcon = () => (
     <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 16 16" fill="none">
          <path d="M10.8538 6.14625C10.9002 6.19269 10.9371 6.24783 10.9623 6.30853C10.9874 6.36923 11.0004 6.43429 11.0004 6.5C11.0004 6.56571 10.9874 6.63077 10.9623 6.69147C10.9371 6.75217 10.9002 6.80731 10.8538 6.85375L7.35375 10.3538C7.30732 10.4002 7.25217 10.4371 7.19147 10.4623C7.13077 10.4874 7.06571 10.5004 7 10.5004C6.9343 10.5004 6.86923 10.4874 6.80853 10.4623C6.74783 10.4371 6.69269 10.4002 6.64625 10.3538L5.14625 8.85375C5.05243 8.75993 4.99972 8.63268 4.99972 8.5C4.99972 8.36732 5.05243 8.24007 5.14625 8.14625C5.24007 8.05243 5.36732 7.99972 5.5 7.99972C5.63268 7.99972 5.75993 8.05243 5.85375 8.14625L7 9.29313L10.1463 6.14625C10.1927 6.09976 10.2478 6.06288 10.3085 6.03772C10.3692 6.01256 10.4343 5.99961 10.5 5.99961C10.5657 5.99961 10.6308 6.01256 10.6915 6.03772C10.7522 6.06288 10.8073 6.09976 10.8538 6.14625ZM14.5 8C14.5 9.28558 14.1188 10.5423 13.4046 11.6112C12.6903 12.6801 11.6752 13.5132 10.4874 14.0052C9.29973 14.4972 7.99279 14.6259 6.73192 14.3751C5.47104 14.1243 4.31285 13.5052 3.40381 12.5962C2.49477 11.6872 1.8757 10.529 1.6249 9.26809C1.37409 8.00721 1.50282 6.70028 1.99479 5.51256C2.48676 4.32484 3.31988 3.30968 4.3888 2.59545C5.45772 1.88122 6.71442 1.5 8 1.5C9.72335 1.50182 11.3756 2.18722 12.5942 3.40582C13.8128 4.62441 14.4982 6.27665 14.5 8ZM13.5 8C13.5 6.9122 13.1774 5.84883 12.5731 4.94436C11.9687 4.03989 11.1098 3.33494 10.1048 2.91866C9.09977 2.50238 7.9939 2.39346 6.92701 2.60568C5.86011 2.8179 4.8801 3.34172 4.11092 4.11091C3.34173 4.8801 2.8179 5.86011 2.60568 6.927C2.39347 7.9939 2.50238 9.09977 2.91867 10.1048C3.33495 11.1098 4.0399 11.9687 4.94437 12.5731C5.84884 13.1774 6.91221 13.5 8 13.5C9.45819 13.4983 10.8562 12.9184 11.8873 11.8873C12.9184 10.8562 13.4983 9.45818 13.5 8Z" fill="#cefb16" />
     </svg>
);

const PricingCard: React.FC<PricingCardProps> = ({
     plan,
     price,
     originalPrice,
     duration,
     description,
     billText,
     planInlude,
     features,
     buttonText,
     buttonDisabled = false,
     highlight = false,
     onClick,
     isDimmed = false,
}) => {
     return (
          <div
               className={`w-full p-[1] rounded-4xl pricing-card-bg transition-all duration-300 ${highlight ? "ring-2 ring-[#cefb16] scale-[1.02]" : ""}`}
          >
               <div className="w-full rounded-4xl p-8">
                    <h5 className="text-xl font-inter font-medium text-white/60 mb-6">
                         {plan}
                    </h5>

                    <div className="flex items-end gap-0.5 mb-3">
                         <div className="flex flex-col">
                              {originalPrice && (
                                   <span className="text-lg text-white/40 line-through mb-1">
                                        {originalPrice}
                                   </span>
                              )}
                              <div className="flex items-end gap-0.5">
                                   <h3 className="text-5xl tracking-[-1px] font-medium text-white">{price}</h3>
                                   <h4 className="text-sm font-inter font-medium text-white">
                                        {duration}
                                   </h4>
                              </div>
                         </div>
                    </div>

                    <p className="text-base font-medium text-white/60 tracking-tighter mb-8">
                         {description}
                    </p>

               <button
                    onClick={(e) => {
                         console.log('🔘 PricingCard button clicked', {
                              plan,
                              hasOnClick: !!onClick,
                              isTrusted: e.isTrusted,
                              type: e.type
                         });
                         onClick?.(e);
                    }}
                    disabled={buttonDisabled}
                    className={`w-full transition inner-shadow-xxl text-base h-10 rounded-xl flex items-center justify-center text-black text-center font-inter font-medium bg-white hover:bg-gray-200 disabled:opacity-50 disabled:cursor-not-allowed disabled:hover:bg-white ${isDimmed ? "opacity-50" : ""}`}
               >
                    {buttonText}
               </button>

                    <h4 className="text-base font-medium font-inter text-white mt-8 mb-6">
                         {planInlude}
                    </h4>
                    <ul className="space-y-2">
                         {features.map((feature, index) => (
                              <li key={index} className="flex items-center gap-3">
                                   <CheckIcon />
                                   <p className="text-sm text-white/60 font-medium font-inter">
                                        {feature.text}{" "}
                                        {feature.subtext && (
                                             <span className="text-[10px]">({feature.subtext})</span>
                                        )}
                                   </p>
                              </li>
                         ))}
                    </ul>
               </div>
          </div>
     );
};

export default PricingCard;
