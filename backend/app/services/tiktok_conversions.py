    ...
    async def send_event(
        ...
        try:
            ...
            if event_name == "AddToCart":
                ...
                logger.info(f"🛒 AddToCart Event Details: value={value}, currency={currency}, content_ids={content_ids}, content_name={content_name}, content_type={content_type}, num_items={num_items}, url={event_url}, user={user.get('email', 'N/A')[:10]}..., external_id={user.get('external_id', 'N/A')}")
                ...
                logger.info(f"  - last_name: {'✓' if user.get('last_name') else '✗'}")
                logger.info(f"  - external_id: {'✓' if user.get('external_id') else '✗'}")
                logger.info(f"📊 AddToCart Context Fields:")
                logger.info(f"  - ip: {'✓' if context.get('ip') else '✗'}")
                logger.info(f"  - user_agent: {'✓' if context.get('user_agent') else '✗'}")
                logger.info(f"  - page.url: {'✓' if context.get('page', {}).get('url') else '✗'}")
                logger.info(f"📦 AddToCart Properties Fields:")
